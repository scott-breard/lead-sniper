#!/usr/bin/env python3
"""
Canadian Restaurant Scraper - New Brunswick Edition
Searches major cities and towns in New Brunswick for fine dining restaurants.
Uses Google Places API with Place Details for complete data.
STRICT GEOGRAPHIC VALIDATION - Only returns restaurants within New Brunswick bounds.
"""
...
# Geographic bounds for New Brunswick (STRICT)
# New Brunswick latitude: ~45.0°N to 48.1°N
# New Brunswick longitude: ~-69.1°W to -63.7°W
# These bounds exclude all neighboring provinces and US states
NB_LAT_MIN = 45.0
NB_LAT_MAX = 48.1
NB_LNG_MIN = -69.1
NB_LNG_MAX = -63.7
...
# File paths
OUTPUT_DIR = "output"
PROGRESS_FILE = "scraper/nb_progress.json"
...
# New Brunswick cities and towns to search
NEW_BRUNSWICK_LOCATIONS = [
    # Major cities
...
PROVINCE_ABBREV = {"New Brunswick": "NB"}
...
def get_output_filename():
    ...
    return os.path.join(OUTPUT_DIR, f"dirona_leads_New_Brunswick_NB_{date_str}.csv")
...
def is_in_new_brunswick(lat, lng):
    """
    STRICT validation that coordinates are within New Brunswick bounds.
    Returns False for ANY location outside these bounds.
    """
    ...
    try:
        ...
        # STRICT check - must be within New Brunswick bounding box
        # This excludes Nova Scotia, Quebec, Maine, and all other regions
        in_bounds = (NB_LAT_MIN <= lat_float <= NB_LAT_MAX and 
                     NB_LNG_MIN <= lng_float <= NB_LNG_MAX)
        
        if not in_bounds:
            # Log filtered coordinates for debugging
            print(f"        ❌ Filtered: lat={lat_float:.4f}, lng={lng_float:.4f} (outside NB)")
...
def parse_place_with_details(place, details, search_location):
    """Parse place into CSV format with STRICT New Brunswick validation."""
    ...
    # **CRITICAL: STRICT geographic filter - ONLY New Brunswick**
    # This is checked TWICE: once before API call, once after
    if not is_in_new_brunswick(lat, lng):
        return None
    ...
    if len(address_parts) >= 3:
        ...
        if "NB" in last_part or "New Brunswick" in last_part:
            province = "NB"
    ...
    # Additional safety check: Reject if province isn't NB
    if province and province != "NB":
        return None
...
def run_scraper():
    """Run the New Brunswick scraper with STRICT geographic validation."""
    ...
    print(f"\n{'='*70}")
    print(f"  New Brunswick Fine Dining Scraper")
    print(f"  STRICT MODE: Only restaurants within NB bounds")
    print(f"{'='*70}")
    print(f"  Locations     : {len(NEW_BRUNSWICK_LOCATIONS)} cities/towns")
    print(f"  Search Query  : {SEARCH_QUERY}")
    print(f"  Filters       : Rating ≥{MIN_RATING} | Price ≥{'$'*MIN_PRICE_LEVEL} | Reviews ≥{MIN_REVIEWS}")
    print(f"  Geo Bounds    : Lat {NB_LAT_MIN}-{NB_LAT_MAX}°N | Lng {NB_LNG_MIN}-{NB_LNG_MAX}°W")
    print(f"  Output File   : {output_file}")
    print(f"  Progress      : {len(completed_locations)}/{len(NEW_BRUNSWICK_LOCATIONS)} locations | {progress.get('total_qualified', 0)} qualified")
    print(f"{'='*70}\n")
    
    total_qualified = 0
    
    for idx, location in enumerate(NEW_BRUNSWICK_LOCATIONS, 1):
        if location in completed_locations:
            continue
        
        print(f"\n[{idx}/{len(NEW_BRUNSWICK_LOCATIONS)}] {location}")
        ...
        for place in places:
            ...
            if is_in_new_brunswick(lat, lng):
                valid_places.append(place)
            else:
                progress["filtered_out_of_bounds"] = progress.get("filtered_out_of_bounds", 0) + 1
        
        if not valid_places:
            print(f"    ⚠️  All results filtered (outside New Brunswick)")
            completed_locations.add(location)
            progress["completed_locations"] = list(completed_locations)
            save_progress(progress)
            time.sleep(API_DELAY)
        ...
        print(f"    ✓ {len(valid_places)} within New Brunswick bounds, fetching details...")
        ...
        for place in valid_places:
            ...
            # Quick check (includes geo validation again)
            ...
            # Parse (includes geo validation AGAIN)
            parsed = parse_place_with_details(place, details, location)
            if parsed:
                seen_place_ids.add(place_id)
                qualified.append(parsed)
                print(f"      ✅ {parsed['Restaurant Name']} | {parsed['City']}, NB | {parsed['Rating']}⭐")
    ...
    print(f"\n\n{'='*70}")
    print(f"  🎉 NEW BRUNSWICK COMPLETE!")
    print(f"{'='*70}")
